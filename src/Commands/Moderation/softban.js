...
module.exports.run = async (bot, msg, args) => {
    ...
    if(toBan == msg.member) return msg.channel.send(error("Bro. You are literally so fucking stupid. Why do I have to deal with people like you. Seriously, use your brain."));
    ...
    if (toBan.roles.highest.position >= msg.member.roles.highest.position) return msg.channel.send(error("This user is above or on the same level on the role hierachy as you, so I won't softban then."));
    ...
    async function softban(member) {
        try {
            ...
            let embed = success(`**${unbanned.tag}** has successfully been softbanned.`).setFooter(`Reason: ${reason}`)
