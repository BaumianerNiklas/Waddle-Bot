...
module.exports.run = async (bot, msg, args) => {
    ...
    let toUnban;
    await msg.guild.fetchBan(args[0])
        .then(b => toUnban = b.user)
        .catch(() => msg.channel.send(error("I can't find that user. Make sure to use an ID of an User that **is actually banned**.")));
    if (!toUnban) return;
    ...
    // Unbanning
    await msg.guild.members.unban(toUnban, auditReason)
    .then(u => {
        let embed = success(`**${u.tag}** has successfully been unbanned.`)
